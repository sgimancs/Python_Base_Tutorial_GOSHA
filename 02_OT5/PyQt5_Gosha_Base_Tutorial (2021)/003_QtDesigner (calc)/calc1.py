# -*- coding: utf-8 -*-
# Реализация формы, созданная при чтении файла пользовательского интерфейса 'calc.ui'
#
# Создано: Генератор кода пользовательского интерфейса PyQt5 5.15.9
#
# ПРЕДУПРЕЖДЕНИЕ: Любые изменения, внесенные в этот файл вручную, будут потеряны, когда pyuic5
# запустить снова. Не редактируйте этот файл, если вы не знаете, что делаете.

# **********************************************************
# QT_003 - Разработка внутри Qt Designer
# ----------------------------------------------------------
# Python 3.13.3
...
class Ui_MainWindow(object):
    ...
    def add_functions(self):
        # передать числа в обработчик событий
        ...
        self.btn_equal.clicked.connect(self.results)    # обработчик событий

    # ------------------------------------------------------
    # OUTPUT
    # ------------------------------------------------------
    def write_number(self, number):
        if self.label_result.text() == "0" or self.is_equal:  # игнорировать первый "0"
        ...
        print(number)   # test (python console)

    # -------------------------------------------------------
    # RESULTS
    # -------------------------------------------------------
    def results(self):
        res = eval(self.label_result.text())    # вычислить (eval)
        self.label_result.setText(" Результат: " + str(res))
        self.is_equal = True  # операция выполнена
        print(self.label_result.text())
